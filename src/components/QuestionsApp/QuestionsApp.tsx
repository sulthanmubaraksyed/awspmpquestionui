...
import ProcessGroupScores from '../ProcessGroupScores';
import OverallStats from '../OverallStats/OverallStats';
import EditResponseDialog from '../EditResponseDialog/EditResponseDialog';
import RetrieveQuestionDialog from '../RetrieveQuestionDialog/RetrieveQuestionDialog';
import QuestionValidityToggle from '../QuestionValidityToggle/QuestionValidityToggle';
...
function QuestionsApp() {
  console.log('🚀 APP: QuestionsApp component initialized');
  ...
  useEffect(() => {
    ...
    // Then add a small delay to ensure the content is fully rendered
    const timer = setTimeout(() => {
      adjustTextareaHeight();
    }, 100);
    
    // And adjust again after a longer delay to catch any late rendering
    ...
    <ErrorBoundary fallback={<div>Something went wrong. Please refresh the page.</div>}>
      <div className="questions-app">
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
            ...
            <>
              <div className="main-content">
                <div className="single-container">
                  <div className="filter-container" style={{ display: 'flex', flexDirection: 'row', gap: '10px', marginBottom: '20px' }}>
                  ...
                  />
                  {/* Only show Sample Toggle for admin users */}
                  {isAdmin && (
                    <SampleToggle
                ...
                <div className="process-scores-container">
                  ...
                  <OverallStats
                    totalAttempted={managerState.responseArray.filter(q => q.is_attempted).length}
                    correctAnswers={managerState.responseArray.filter(q => q.is_attempted && q.did_user_get_it_right === true).length}
                  />
                  {/* Only show Validity Toggle for admin users */}
                  {isAdmin && (
          ...
          {/* EditResponseDialog is available to both admin and guest users */}
          ...
          {/* RetrieveQuestionDialog is only available to admin users */}
          {isAdmin && (
